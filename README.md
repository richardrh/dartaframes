...
`dartaframes_polars` runs lazy Polars queries through a Rust native library and brings
...
The package and native binaries are published as `dartaframes_polars` 0.1.0. Install
...
`package:dartaframes_polars/polars.dart` re-exports it for Polars applications. Read
...
```dart
import 'package:dartaframes_polars/arrow.dart'; // Pure Dart, no native library.
```
...
```dart
import 'package:dartaframes_polars/polars.dart'; // Polars plus the Arrow value API.
import 'package:dartaframes_polars/arrow.dart';  // Arrow values without Polars APIs.
```
